"""Data/editorial invariants. Coverage gaps are reported, never silently passed as research."""
...
class Integrity(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.manifest=load(D/'manifest.json')
        cls.raw=load(D/'candidates-official.json')
        cls.dataset=load(P/'dados.json')
        cls.records=cls.dataset['candidates']
        cls.soup=BeautifulSoup((P/'index.html').read_text(),'html.parser')
        cls.report=load(A/'build-report.json')
        cls.profiles=load(D/'profiles-official.json')
    ...
    def test_exact_census_projection(self):
        ...
        self.assertEqual(expected,{c['id'] for c in self.records})
    ...
    def test_scope_state_office_year(self):
        ...
        self.assertEqual(self.dataset['state'],'RS')
        self.assertEqual(self.dataset['election_year'],2026)
    ...
    def test_identifiers_and_numbers(self):
        self.assertEqual(len({c['id'] for c in self.records}),len(self.records))
        self.assertEqual(len({c['number'] for c in self.records}),len(self.records))
        raw={r['SQ_CANDIDATO']:r for r in self.raw}
        for c in self.records:
            self.assertRegex(c['number'],r'^\d{5}$')
            self.assertTrue(c['number'].startswith(raw[c['id']]['NR_PARTIDO']))
    ...
    def test_all_individual_profiles_read(self):
        ...
        for c in self.records:
            p=self.profiles[c['id']]
            self.assertEqual(str(p['data']['id']),c['id'])
            self.assertEqual(str(p['data']['numero']),c['number'])
            self.assertEqual(str(p['data']['cargo']['codigo']),'7')
            self.assertEqual(p['data']['partido']['sigla'].upper(),c['party'].upper())
            self.assertEqual(p['data']['descricaoSituacao'],c['status'])
            self.assertTrue(c['status_checked_at'])

    def test_reconciliation_and_format_notes(self):
        audit=load(A/'reconciliation.json')
        self.assertEqual(audit['identity_mismatches'],[])
    ...
    def test_photos_are_official_and_local(self):
        for c in self.records:
            self.assertIsNotNone(c['photo'])
            p=c['photo'];path=P/p['path']
            self.assertTrue(path.is_file())
            self.assertEqual(hashlib.sha256(path.read_bytes()).hexdigest(),p['sha256'])
            self.assertIn(c['id'],p['source_member'])
            self.assertIn('cdn.tse.jus.br',p['source_url'])

    def test_all_candidates_rendered_exactly_once(self):
        cards=self.soup.select('article.candidate')
        self.assertEqual({c['data-tse-id'] for c in cards},{c['id'] for c in self.records})
        self.assertEqual(len(cards),len(self.records))
        ids=[n['id'] for n in self.soup.select('[id]')]
    ...
    def test_no_current_election_results_or_votes(self):
        for c in self.records:
            for h in c['history']:
                if h['year']==2026:
                    self.assertIsNone(h['votes'])
                    self.assertNotIn(str(h.get('result','')).lower(),('eleito','suplente','nao eleito'))
        ...
        for c in self.records:
            for h in c['history']:
                if h.get('votes') is not None:
                    self.assertEqual(h['uf'],'RS')
                    self.assertLess(h['year'],2026)
                    self.assertFalse(h.get('round_note'))
                    self.assertFalse(str(h.get('office','')).upper().startswith(('VICE','SUPLENTE')))
                    source=sources[str(h['year'])]
                    self.assertEqual(h['votes'],source['totals'][h['candidate_id']+':'+str(h['round'])])
                    self.assertIsNone(source['archive_sha256'])
                    self.assertRegex(source['member_sha256'],r'^[a-f0-9]{64}$')
    ...
    def test_agendas_and_acts_require_reviewed_sources(self):
        ...
        for c in self.records:
            if c['pautas'] or c['activities']:
                self.assertTrue(editorial[c['id']]['reviewed'])
                self.assertTrue(c['editorial_sources'])
                self.assertTrue(c['editorial_checked_at'])
            for activity in c['activities']:
                self.assertTrue(activity['source'])
                self.assertTrue(activity['text'])

    def test_offices_require_current_institutional_evidence(self):
        for c in self.records:
            if c['current_office']:
                office=c['current_office']
                self.assertTrue(office['checked_at'])
                self.assertTrue(office['source'])
                self.assertNotIn('tse.jus.br',office['source'])
                self.assertIn(office['verification'],('current_directory','current_individual_profile'))

    def test_unknown_office_is_not_claimed_absent(self):
        for c in self.records:
            if not c['current_office']:
                article=self.soup.find(id='candidato-'+c['id'])
                self.assertIn('Não confirmado nesta edição',article.get_text())
                self.assertIn('não equivale à ausência de mandato',article.get_text())

    def test_all_records_have_explicit_checks(self):
        for c in self.records:
            self.assertTrue(c['checks']['registro_validado'])
            self.assertTrue(c['checks']['historico_fonte_consultada'])
            self.assertTrue(c['checks']['links_declarados_auditados'])
            self.assertEqual(c['checks']['pautas_com_fonte_revisada'],bool(c['pautas']))

    def test_private_registration_fields_not_exported(self):
        serialized=(P/'dados.json').read_text().lower()
        for key in ('nr_cpf','cpf_candidato','nr_titulo_eleitoral','email_candidato','nr_cnpj','dt_nascimento','ds_cor_raca','ds_genero','endereco_residencial'):
    ...
    def test_links_safe_and_internal_targets_exist(self):
        for a in self.soup.select('a[href]'):
            href=a['href']
            self.assertFalse(href.lower().startswith(('javascript:','data:')))
    ...
    def test_canonical_and_structured_data(self):
        ...
        self.assertEqual(self.soup.select_one('link[rel="canonical"]')['href'],canonical)
        self.assertIn('Deputado Estadual',self.soup.title.text)
        ...
        self.assertNotIn('position',json.dumps(ld))
        self.assertEqual(ld['url'],canonical)

    def test_exports_and_opengraph_exist(self):
        from PIL import Image
        for name in ('dados.json','fontes.json','cobertura.json','candidaturas.csv','site.webmanifest','sitemap.xml','favicon.svg','assets/app.js'):
    ...
    def test_editorial_gaps_are_quantified(self):
        gaps=[c['id'] for c in self.records if not c['pautas']]
        self.assertEqual(gaps,self.report['editorial_gap_ids'])
        self.assertEqual(self.report['with_editorial_summary'],len(self.records)-len(gaps))
        self.assertEqual(self.report['editorial_gate'],'complete' if not gaps else 'documented_gaps')
