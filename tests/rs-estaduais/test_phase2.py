"""Regression tests for A-D enrichment; these are not a claim of final E/F review."""
...
class PhaseTwo(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.records=load(D/'normalized.json')['candidates']
        cls.raw={r['SQ_CANDIDATO']:r for r in load(D/'candidates-official.json')}
        cls.history=load(D/'history-normalized.json')
        cls.baseline=load(A/'baseline.json')
    ...
    def test_original_census_unchanged(self):
        ...
        self.assertEqual(self.baseline['historical_vote_rows'],326)

    def test_all_old_year_sources_have_context_revalidation(self):
        sources=load(D/'votes-phase2.json')
        self.assertEqual(set(sources),{'2004','2006','2008','2010'})
        for year,source in sources.items():
            self.assertEqual(source['matching_version'],2,year)
            self.assertIsNone(source['archive_sha256'])
            self.assertRegex(source['member_sha256'],r'^[a-f0-9]{64}$')
            self.assertEqual(source['member_bytes'],source['member_bytes'])
            self.assertGreater(source['member_bytes'],0)
    ...
    def test_old_votes_match_linked_electoral_unit_and_office(self):
        ...
        for cid,rows in self.history.items():
            for h in rows:
                ...
                expected=raw[(cid,h['year'],h['candidate_id'],h['round'])]
                source=sources[str(h['year'])]
                key=h['candidate_id']+':'+str(h['round'])
                ctx=source['contexts'][key]
                self.assertEqual(ctx['electoral_unit'],expected['SG_UE'])
                self.assertEqual(ctx['office_code'],expected['CD_CARGO'])
                self.assertEqual(ctx['ballot_number'],expected['NR_CANDIDATO'])
                self.assertEqual(ctx['year'],h['year'])
                self.assertEqual(ctx['round'],h['round'])
                self.assertEqual(ctx['uf'],'RS')
                self.assertEqual(source['totals'][key],h['votes'])

    def test_reused_2004_ids_excluded(self):
        source=load(D/'votes-phase2.json')['2004']
        self.assertGreater(source['excluded_reused_id_rows'],0)
        self.assertEqual(len(source['totals']),19)
    ...
    def test_ticket_and_other_population_votes_never_assigned(self):
        for hs in self.history.values():
            for h in hs:
                if h['year']>=2026:
                    self.assertIsNone(h.get('votes'));continue
                reason=self.votes_module.reason(h)
                if reason:self.assertIsNone(h.get('votes'),(h['year'],h['office']))
    ...
    def test_phase2_identities_and_source_dates(self):
        ...
        for cid,patch in patches.items():
            ...
            self.assertTrue(patch['reviewed'])
            self.apply_module.valid_date(patch['checked_at'])
            for source in patch['sources']:
                self.assertTrue(source['locator']);self.assertTrue(source['type'])
                self.apply_module.valid_url(source['url'])
    ...
    def test_phase2_merge_is_idempotent(self):
        ...
        original=copy.deepcopy((editorial,offices))
        patches,_=self.apply_module.batches('phase2-editorial')
        office_patches,_=self.apply_module.batches('phase2-offices')
        self.apply_module.apply(editorial,offices,patches,office_patches,self.raw)
        self.apply_module.apply(editorial,offices,patches,office_patches,self.raw)
        self.assertEqual((editorial,offices),original)

    def test_wrong_candidate_identity_rejected(self):
        patches,_=self.apply_module.batches('phase2-editorial')
        cid=next(iter(patches));patch=copy.deepcopy(patches[cid]);patch['expected_name']='IDENTIDADE DIVERGENTE PARA TESTE'
        with self.assertRaisesRegex(ValueError,'identity mismatch'):
            self.apply_module.apply({}, {}, {cid:patch}, {}, self.raw)

    def test_future_source_rejected(self):
        patches,_=self.apply_module.batches('phase2-editorial')
        cid=next(iter(patches));patch=copy.deepcopy(patches[cid]);patch['sources'][0]['published_at']='2026-09-22'
        with self.assertRaisesRegex(ValueError,'beyond declared cutoff'):
            self.apply_module.apply({}, {}, {cid:patch}, {}, self.raw)

    def test_conflicting_summary_rejected(self):
        patches,_=self.apply_module.batches('phase2-editorial')
        cid=next(k for k,v in patches.items() if v.get('pautas'))
        with self.assertRaisesRegex(ValueError,'Conflicting reviewed pautas'):
            self.apply_module.apply({cid:{'pautas':'Conteúdo de teste que deve ser preservado.'}}, {}, {cid:patches[cid]}, {}, self.raw)

    def test_unreviewed_input_rejected(self):
        patches,_=self.apply_module.batches('phase2-editorial')
        cid=next(iter(patches));patch=copy.deepcopy(patches[cid]);patch['reviewed']=False
    ...
    def test_all_current_office_inputs_have_primary_scope(self):
        ...
        for patch in patches.values():
            self.assertIn(patch['verification'],('current_directory','current_individual_profile'))
            self.assertTrue(patch['directory_scope'].startswith('current'))
            self.assertIn('2026',patch['checked_at'])
            self.assertTrue(patch['detail'])
