...
PARTY = {p.upper(): p for p in PARTIES}
ARCHIVE = 'https://cdn.tse.jus.br/estatistica/sead/odsele/'
URLS = {
 'candidates': ARCHIVE+'consulta_cand/consulta_cand_2026.zip',
 'status': ARCHIVE+'consulta_cand_complementar/consulta_cand_complementar_2026.zip',
 'social': ARCHIVE+'consulta_cand/rede_social_candidato_2026.zip',
 'history': ARCHIVE+'historico_candidatura/historico_candidatura_2026.zip',
 'photos': 'https://cdn.tse.jus.br/estatistica/sead/eleicoes/eleicoes2026/fotos/foto_cand2026_PR_div.zip',
 'map': 'https://servicodados.ibge.gov.br/api/v3/malhas/estados/41?formato=image/svg&qualidade=minima'
}
...
def get(url,timeout=60):
 ...
 for attempt in range(3):
  try:
   ...
   errors.append(str(exc)); time.sleep(attempt+1)
 raise RuntimeError('; '.join(errors))
def ziprows(raw):
 archive=zipfile.ZipFile(io.BytesIO(raw))
 names=[n for n in archive.namelist() if n.upper().endswith('_PR.CSV')]
 if not names:
  csvnames=[n for n in archive.namelist() if n.lower().endswith('.csv')]
  names=[n for n in csvnames if 'BRASIL' in n.upper()] or (csvnames if len(csvnames)==1 else [])
...
def collect():
 ...
 if not manifest:
  manifest={'schema_version':1,'state':'PR','election_year':2026,'collected_at':now(),'scope':PARTIES,'scope_rule':'Recorte de oito siglas herdado de SC/RS; não é o total do estado nem uma avaliação individual.','sources':{},'errors':[]}
 if not (DATA/'candidates-official.json').exists():
  ...
  selected=[r for r in universe if r.get('SG_PARTIDO','').upper() in PARTY]
  ...
  manifest['sources']['candidates']={'url':URLS['candidates'],'sha256':hashlib.sha256(raw).hexdigest(),'checked_at':now(),**meta}
  manifest['census']={}
 ...
 for kind in ('status','social','history'):
  ...
  try:
   ...
   if kind=='history':
    selected=[r for r in rows if r.get('SQ_CANDIDATO_ATUAL') in ids]
    allowed=HISTORY_KEEP
    assert 'SQ_CANDIDATO_ATUAL' in rows[0],'Historical identity link missing'
   else:
    selected=[r for r in rows if r.get('SQ_CANDIDATO') in ids]
    allowed=['SQ_CANDIDATO','DT_GERACAO','HH_GERACAO']
   ...
   save(target,[{k:r[k] for k in allowed if k in r} for r in selected])
   manifest['sources'][kind]={'url':URLS[kind],'sha256':hashlib.sha256(raw).hexdigest(),'checked_at':now(),'selected_rows':len(selected),**meta}
  except Exception as exc:manifest['errors'].append({'stage':kind,'at':now(),'error':str(exc)})
  save(DATA/'manifest.json',manifest)
 if not (DATA/'photos-official.json').exists():
  try:
   ...
   save(DATA/'photos-official.json',matched)
   manifest['sources']['photos']={'url':URLS['photos'],'sha256':hashlib.sha256(raw).hexdigest(),'checked_at':now(),'matched':len(matched)}
  except Exception as exc:manifest['errors'].append({'stage':'photos','at':now(),'error':str(exc)})
 if not (ASSETS/'pr-ibge.svg').exists():
  try:
   raw=get(URLS['map']);assert b'<svg' in raw
   (ASSETS/'pr-ibge.svg').write_bytes(raw)
   manifest['sources']['map']={'url':URLS['map'],'sha256':hashlib.sha256(raw).hexdigest(),'checked_at':now()}
  except Exception as exc:manifest['errors'].append({'stage':'map','at':now(),'error':str(exc)})
 save(DATA/'manifest.json',manifest)
 profiles=read(DATA/'profiles-official.json',{})
 election_ids={r['CD_ELEICAO'] for r in records};assert len(election_ids)==1
 election_id=next(iter(election_ids))
 def profile(row):
  ...
  try:
   raw=get(url,18);data=json.loads(raw);assert str(data.get('id'))==cid
   ...
   public={k:data[k] for k in allowed if k in data}
   return cid,{'data':public,'url':url,'sha256':hashlib.sha256(raw).hexdigest(),'checked_at':now()}
 ...
 manifest['profiles_read']=sum('data' in p for p in profiles.values());manifest['profiles_failed']=sum('error' in p for p in profiles.values());manifest['finished_at']=now()
 save(DATA/'manifest.json',manifest)
