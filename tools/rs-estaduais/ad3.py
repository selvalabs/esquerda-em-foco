...
def report():
    ...
    protected=['index.html','deputados-estaduais','rs/deputados-federais','data/rs','tools/rs','assets','server','sitemap.xml','config','docs/rs-estaduais/ad2-close']
    for path in protected:subprocess.run(['git','diff','--exit-code',BASE,'--',path],cwd=ROOT,check=True,capture_output=True)
    obj['isolation_passed']=True;obj['protected_paths']=protected
