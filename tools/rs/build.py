...
PARTIES=json.loads((ROOT/'config/party-scope-2026.json').read_text(encoding='utf-8'))['parties']
...
def build():
 ...
 soup.select_one('.site-nav__edition').string='RS · 2026';soup.select_one('h1.title').string='Rio Grande do Sul';soup.select_one('.dek').string='Candidaturas do recorte partidário canônico de 2026, reunidas por sigla e acompanhadas de fontes públicas. A presença nesta página não é recomendação de voto.'
 ...
 source_cards=''.join(f'<article><p class="eyebrow">{esc(name)}</p><p>{text}</p>{citation(key)}</article>' for key,name,text in [('cadastro','Cadastro oficial','Recorte: eleição 2026, UF RS, cargo 6 (deputado federal), nas 11 siglas do recorte canônico explicitadas em Sobre. Cada ficha informa seu identificador.'),('situacao','Situação eleitoral','A situação é um retrato do arquivo consultado. Registros com renúncia permanecem identificados, não apresentados como candidaturas ativas.'),('historico','Histórico eleitoral','Disputas vinculadas são exibidas com ano, cargo e resultado quando disponíveis. Votos conferidos têm referência própria. Não inferimos posse ou mandato atual apenas a partir de eleição anterior.'),('redes','Redes e sites declarados','Endereços são normalizados tecnicamente. Entradas incompletas ou inseguras não viram links inventados. A declaração não garante disponibilidade contínua do site.'),('fotos','Fotografias oficiais','Fotografias locais provêm do pacote oficial, associadas pelo identificador TSE. Imagens ausentes teriam iniciais como alternativa, nunca retratos gerados.')])
...
''';soup.head.append(style)
 ...
 write(DEST/'fontes.json',{'official_snapshot':manifest,'verification':extra,'editorial':read('editorial.json',{}),'scope_note':'Recorte canônico de 11 siglas de 2026; siglas sem registro permanecem explicitadas na auditoria.'})
