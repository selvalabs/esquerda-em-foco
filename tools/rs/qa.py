"""Actual HTTP browser checks, served both below a Pages prefix and a VPS root."""
from __future__ import annotations
import contextlib, functools, hashlib, http.server, json, os, threading, time
...
def overflow(page):return page.evaluate('({width:innerWidth,scroll:document.documentElement.scrollWidth,body:document.body.scrollWidth})')
def no_overflow(page,name):
 sizes=overflow(page);check(name,sizes['scroll']<=sizes['width']+1 and sizes['body']<=sizes['width']+1,sizes)
...
try:
 with sync_playwright() as p:
  ...
  for width in [320,360,390,430,768,1024,1440]:
   ...
   # No external fonts or tracking are required by the page. Only its own files are loaded.
   ...
   page.locator('.candidate').first.scroll_into_view_if_needed();page.wait_for_timeout(100);no_overflow(page,f'card overflow {width}')
   ...
   page.locator('.persistent-footer').scroll_into_view_if_needed();no_overflow(page,f'footer overflow {width}')
   ...
   page.locator('#searchInput').fill('zzzz-sem-candidato-xyz');check(f'empty result {width}',page.locator('.candidate:visible').count()==0 and page.locator('#emptyResults').is_visible());check(f'counter zero {width}',page.locator('#resultCount').inner_text()=='0 resultados')
  ...
  check('Brasilia date independent of client timezone',before_midnight==first)
  browser.close()
