...
(function (root) {
  ...
  function boot() {
    ...
    function change(replace=false){clearTimeout(timer);state.ignored=[];render();writeURL(replace);}
    ...
    root.addEventListener('popstate',()=>{clearTimeout(timer);state=parseState(location.search,valid);render();});
