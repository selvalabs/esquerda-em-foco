...
| Sínteses de pautas ou temas com fonte | 30 | 61 | +31 |
| Candidaturas ainda sem síntese | 115 | 88 | -27 |
| Mandatos atuais confirmados | 5 | 16 | +11 |
| Fichas com atos ou registros institucionais | 15 | 31 | +16 |
...
Foram acrescentadas 30 sínteses às 30 existentes. As adições usam páginas individuais, programas legíveis, entrevistas e registros institucionais identificados. Restam 88 candidaturas sem síntese temática. Não se afirma busca exaustiva de todas elas. A situação de cada ficha e os próximos passos estão em `candidate-coverage.csv` e `.json`.
...
Foram acrescentadas 10 confirmações a partir de diretórios institucionais de Santa Maria, Passo Fundo e Caxias do Sul. O total confirmado nesta base é 16. Resultados eleitorais antigos, autodescrição de campanha e uma notícia pontual não foram convertidos em confirmação de exercício atual.
...
A cobertura passou de 15 para 31 fichas com atos ou registros institucionais. As adições incluem proposições, relatorias, composição de comissões, reuniões de frentes, pronunciamentos e pedidos de informação. Cada adição tem fonte e data; notícia de gabinete é identificada como tal.
...
Testes unitários: **92**. Verificações Chromium: **63**. Resultado combinado: **passed**. O workflow verifica o isolamento das frentes existentes. Esses testes são regressões do incremento A–D, não declaração de encerramento da revisão editorial independente E/F.
