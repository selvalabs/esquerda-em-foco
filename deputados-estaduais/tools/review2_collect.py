#!/usr/bin/env python3
"""Fresh, transactional public-data collection. Does not touch the published page.
Run on a review branch; snapshots and manifests contain no private identifiers.
"""
...
def public(row): return {k:v for k,v in row.items() if not any(x in k for x in PRIVATE)}
def key(r): return r['SQ_CANDIDATO']
def state(r): return r.get('DS_SITUACAO_JULGAMENTO') or r.get('DS_SITUACAO_CANDIDATO_TOT') or 'Não informado'
def main():
 ...
 if not baseline_path.exists():
  ...
  hashes={p:digest(REPO/p) for p in tracked if p and not p.startswith('deputados-estaduais/') and not p.startswith('.github/workflows/estaduais-review2')}
  save(baseline_path, {'commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=REPO).decode().strip(), 'outside_state_sha256':hashes, 'state_data_sha256':digest(ROOT/'data/candidaturas.json'), 'profiles':load(ROOT/'data/candidaturas.json')})
 now=datetime.now(timezone.utc).isoformat()
 report={'consulted_at':now,'election_year':2026,'uf':'SC','office_code':'7','fresh_requests':True,'sources':{},'errors':[]}
 downloaded={}; ids=set()
 try:
  for filename,url in SOURCES.items():
   ...
   with z:
    ...
    if len(names)!=1: raise ValueError(f'Expected exactly one SC member in {url}: {names}')
    with z.open(names[0]) as f:
     for r in csv.DictReader(io.TextIOWrapper(f,encoding='latin-1',newline=''),delimiter=';'):
      if filename.startswith('universo'):
       include=r.get('SG_UF')=='SC' and r.get('CD_CARGO')=='7' and r.get('ANO_ELEICAO')=='2026'
      else:
       include=(r.get('SQ_CANDIDATO_ATUAL') or r.get('SQ_CANDIDATO')) in ids
      if include:
       rows.append(public(r)); generations.add((r.get('DT_GERACAO',''),r.get('HH_GERACAO','')))
   if filename.startswith('universo'):
    if not rows or len({key(r) for r in rows})!=len(rows): raise ValueError('Empty or duplicated official state-deputy universe')
    ids={key(r) for r in rows if r['SG_PARTIDO'].upper() in PARTIES}
    if not ids: raise ValueError('Empty party scope; manual review required')
  ...
  # No partial snapshot is presented as a successful fresh collection.
  ...
  diff={'before_count':len(old),'after_count':len(new),'universe_count':len(downloaded['universo-sc-2026.json']),'retained':len(old.keys() & new.keys()),'added':[{'id':s,'name':new[s]['NM_URNA_CANDIDATO']} for s in sorted(new.keys()-old.keys())],'absent':[{'id':s,'name':old[s]['NM_URNA_CANDIDATO']} for s in sorted(old.keys()-new.keys())],'changes':changes,'registration_statuses':dict(Counter(state(r) for r in newstatus.values())),'by_party':dict(Counter(r['SG_PARTIDO'] for r in new.values()))}
  ...
  # Compact review queue; only metadata, never full third-party articles.
  existing={p['id']:p for p in load(ROOT/'data/candidaturas.json')}
  ...
  for start in range(0,len(ordered),12):
   ...
   for r in ordered[start:start+12]:
    ...
    hist=p.get('history',[])
    selected=[h for h in hist if str(h.get('result','')).lower().startswith('eleit') or h.get('year') in (2022,2024)]
  ...
  report.update(passed=False); report['errors'].append(str(exc)); raise
