#!/usr/bin/env python3
"""Reconcile all linked elections with contextual joins, including older short IDs."""
from __future__ import annotations
import csv, io, json, re, unicodedata
from collections import defaultdict
from concurrent.futures import ThreadPoolExecutor
from datetime import datetime, timezone
from pathlib import Path
from http_archive import archive
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'audit/review2'
BASE='https://cdn.tse.jus.br/estatistica/sead/odsele/votacao_candidato_munzona/'
def load(p): return json.loads(p.read_text(encoding='utf-8'))
def save(p,v): p.write_text(json.dumps(v,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
def norm(s): return ''.join(c for c in unicodedata.normalize('NFD',str(s).upper()) if unicodedata.category(c)!='Mn' and c.isalnum())
def num(v):
 try:return str(int(v))
 except (TypeError,ValueError):return str(v or '')
def pick(r,*keys):
 return next((r[k] for k in keys if k in r),'')
def identity(r,year=None):
 return (num(year or pick(r,'ANO_ELEICAO')),num(pick(r,'SG_UE','CODIGO_UE')),num(pick(r,'CD_CARGO','CODIGO_CARGO')),num(pick(r,'NR_CANDIDATO','NUMERO_CANDIDATO')),num(pick(r,'NR_TURNO','NUM_TURNO')))
def vote_key(r):
 return ':'.join([str(r['ANO_ELEICAO']),str(r['SQ_CANDIDATO']),str(r['NR_TURNO']),str(r['SG_UE']),str(r['CD_CARGO']),str(r['NR_CANDIDATO'])])
def legacy_key(r):
 return f"{r['ANO_ELEICAO']}:chapa:{r['SG_UE']}:{r['NR_CANDIDATO']}:{r['NR_TURNO']}" if r['CD_CARGO'] in ['4','12'] else f"{r['ANO_ELEICAO']}:{r['SQ_CANDIDATO']}:{r['NR_TURNO']}"
def main():
 h=load(OUT/'snapshot/historico-sc-2026.json')
 grouped=defaultdict(dict)
 for r in h:
  if int(r['ANO_ELEICAO'])<2026:grouped[r['ANO_ELEICAO']][vote_key(r)]=r
 old=load(ROOT/'data/votos-historicos.json')
 def collect_year(item):
  year,targets=item; url=BASE+f'votacao_candidato_munzona_{year}.zip'; found={}; by_context=defaultdict(list)
  for k,r in targets.items():
   ident=list(identity(r)); ticket=r['CD_CARGO'] in ['4','12']
   if ticket:ident[2]='3' if r['CD_CARGO']=='4' else '11'
   by_context[tuple(ident)].append((k,r,ticket))
  meta={'url':url,'year':year,'target_elections':len(targets),'read_complete':False}
  try:
   z,m=archive(url);meta.update(m)
   with z:
    names=[n for n in z.namelist() if re.search(r'_SC\.csv$',n,re.I)]
    if len(names)!=1:raise ValueError('Expected one SC member')
    meta['member']=names[0]
    with z.open(names[0]) as f:
     reader=csv.DictReader(io.TextIOWrapper(f,encoding='latin-1',newline=''),delimiter=';')
     meta['columns']=reader.fieldnames; consumed=0
     for row in reader:
      consumed+=1
      if pick(row,'SG_UF','SIGLA_UF') not in ('','SC'):continue
      matches=by_context.get(identity(row,year),[])
      for k,target,ticket in matches:
       sid=pick(row,'SQ_CANDIDATO','SEQUENCIAL_CANDIDATO')
       name=pick(row,'NM_CANDIDATO','NOME_CANDIDATO')
       exact_id=num(sid)==num(target['SQ_CANDIDATO'])
       exact_name=bool(name) and norm(name)==norm(target.get('NM_CANDIDATO',''))
       if not ticket and not exact_id and not exact_name:continue
       amount=pick(row,'QT_VOTOS_NOMINAIS','QTDE_VOTOS')
       if not re.fullmatch(r'\d+',str(amount)):raise ValueError('Non-integer vote count')
       if k not in found:found[k]={'year':year,'candidate_id':target['SQ_CANDIDATO'],'source_candidate_id':sid,'round':target['NR_TURNO'],'electoral_unit':target['SG_UE'],'office_code':target['CD_CARGO'],'number':target['NR_CANDIDATO'],'votes':0,'rows':0,'source':url,'type':'chapa' if ticket else 'nominal','match_method':'year+UF+electoral_unit+office+number+round+'+('ticket' if ticket else 'candidate_id' if exact_id else 'exact_full_name')}
       found[k]['votes']+=int(amount);found[k]['rows']+=1
     meta.update(read_complete=True,source_rows=consumed,matched=len(found))
  except Exception as exc:meta['error']=str(exc)
  print('Contextual votes',year,len(found),'/',len(targets),meta.get('error','OK'),flush=True)
  return year,found,meta
 results={};sources={}
 with ThreadPoolExecutor(max_workers=3) as pool:
  for year,values,meta in pool.map(collect_year,sorted(grouped.items())):
   results.update(values);sources[year]=meta
 changes=[];gaps=[]
 for year,targets in sorted(grouped.items()):
  for k,r in targets.items():
   previous=old.get(legacy_key(r),{}).get('votes');now=results.get(k,{}).get('votes')
   detail={'current_id':r['SQ_CANDIDATO_ATUAL'],'historical_id':r['SQ_CANDIDATO'],'name':r.get('NM_URNA_CANDIDATO'),'year':int(year),'unit':r['SG_UE'],'office':r['CD_CARGO'],'number':r['NR_CANDIDATO'],'previous_votes':previous,'reconciled_votes':now}
   if now is None:gaps.append(detail)
   if previous!=now:changes.append(detail)
 report={'consulted_at':datetime.now(timezone.utc).isoformat(),'all_archives_read':all(x.get('read_complete') for x in sources.values()),'sources':sources,'targets':sum(len(x) for x in grouped.values()),'matched':len(results),'changes':changes,'gaps':gaps,'rule':'Contextual join. Missing total is null, never zero. Vice candidates receive ticket votes, explicitly identified.'}
 save(OUT/'votes-contextual.json',results);save(OUT/'votes-audit.json',report)
 print(json.dumps({k:v for k,v in report.items() if k!='sources'},ensure_ascii=False,indent=2))
if __name__=='__main__':main()
