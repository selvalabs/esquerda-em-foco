...
(function () {
  'use strict';
  const data = window.EEF_SC_FEDERAIS_FILTERS;
  ...
  if (!data || !core || !panel || !search || !count || !dialog || !opener || !sidebar || !active || !empty) return;
  ...
  const records = cards.map(card => {
    ...
    copy.querySelectorAll('details, .candidate-index, .pauta-match').forEach(node => node.remove());
    const candidate = candidateMap.get(card.dataset.tseId);
    return {id: candidate.id, topicIds: candidate.topicIds, text: copy.textContent, card, candidate};
  });
  ...
  function matchingEvidence(record) {
    ...
    if (!box) {
      ...
      record.card.querySelector('.pauta').insertAdjacentElement('afterend', box);
    }
    const groups = new Map();
    record.candidate.matches.filter(match => selected.has(match.topicId)).forEach(match => {
      const key = JSON.stringify([match.direction, match.target, match.sourceIds]);
      if (!groups.has(key)) groups.set(key, {...match, topicIds: []});
      groups.get(key).topicIds.push(match.topicId);
    });
    box.replaceChildren(node('p', 'pauta-match__heading', 'Neste filtro'));
    const list = node('ul', 'pauta-match__list');
    groups.forEach(match => {
      const item = node('li');
      item.append(node('strong', '', match.topicIds.map(id => topicMap.get(id).label).join(' · ')));
      const prefix = match.direction === 'apoio' ? 'Apoio declarado: ' : 'Prioridade declarada: ';
      item.append(node('span', '', prefix + match.target + '.'));
      const refs = node('span', 'pauta-match__sources');
      match.sourceIds.forEach((id, index) => {
        const source = data.sources[id];
        if (!source) return;
        const a = node('a', '', `Fonte ${index + 1} ↗`);
        a.href = source.url;
        a.target = '_blank';
  ...
  function render() {
    ...
    panel.querySelector('[data-pauta-selection]').textContent = selected.size
      ? `${selected.size} pauta${selected.size === 1 ? '' : 's'} selecionada${selected.size === 1 ? '' : 's'}`
